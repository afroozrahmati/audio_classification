...
def model_training(model,x_train,y_train,epochs):
...
def model_evaluate(model,x_train,y_train,x_test,y_test,epochs):
    ...
    with open('D:\\UW\\Final thesis\\audio_classification\\result_fl.csv', 'a') as f:
        f.write(s)
...
pre=preprocessing()
x_train, x_test, y_train, y_test = pre.load_processed_data()
...
for comm_round in range(comms_round):
    ...
    for (client_name, data) in clients.items():
        ...
        local_model = model_training(local_model, data[0], data[1],2)
